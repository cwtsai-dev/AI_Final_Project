...
def generate_corr_matrices(market='hs300', years=('2018', '2019', '2020', '2021', '2022', '2023', '2024')):
    ...
    for i in range(len(dt)):
        ...
        # check whether it is already the last day of the month
        end_date = pd.to_datetime(end_date)
        if end_date.day == end_date.replace(day=1).days_in_month:
            relation_dt = end_date
        else:
            relation_dt = end_date + MonthEnd(1)    # last calendar day of the month (not the last trading day)
...
if __name__ == '__main__':
    import sys
    market = sys.argv[1] if len(sys.argv) > 1 else 'hs300'
